...
<table width="80%">
  <tr>
    <td width="50%">
      ...
      $strPayHeaderTable = "payhireheader";
      $strPayHeaderCondition = "emp_id='".$_COOKIE['uid']."'";
      $strPayHeaderSort = " order by id";
      $PHList = fncSelectConditionRecord($strPayHeaderTable,$strPayHeaderCondition,$strPayHeaderSort);
      while($PH = mysql_fetch_array($PHList)){
        $statusConfirm = "<span style=\"color:#aaa\">รอการตรวจสอบ</span>";
        if($PH['confirmstatus']>0){
          $statusConfirm = "<span style=\"green\" class=\"button\">ยืนยันยอด</span>";
          if($PH['confirmstatus']<9999){
            $statusConfirm = "<span style=\"color:#111\">ยืนยันแล้ว</span>";
          }
        }
        echo "<p class=\"link2checkcaltopay\" for=\"".$PH['id']."\">".$PH['headername']." : สถานะ ".$statusConfirm."</p>";
      }
      ?>
      <table width="100%">
...
*/
				echo "IN66666";
				$table1 = "payhireheader";
				$condition1 = "id = ".$phid;
				$emp1 = fncSelectSingleRecord($table1,$condition1);
				$emp_id1 = $emp1['emp_id'];
				echo $emp_id1."ID";
        $sumTotal = 0;
